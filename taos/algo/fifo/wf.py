...
def WF(jobs, sites):
    ...
    with tqdm(total=total) as pbar:
        pbar.set_description("WF progress:")
        t = 0
        while True:
            ...
            for job in jobs:
                if job.arrival_time == t:
                    ...
                    s = time.time()
                    # Call wf() to obtain the task assignment solution
                    _ = common.wf(job=job, tmp_bklgs=tmp_bklgs, solution=solution)
                    e = time.time()
