"""
The Replica-Deletion Heuristic.
"""
...
def RD(jobs, sites):
    ...
    with tqdm(total=total) as pbar:
        pbar.set_description("RD progress:")
        t = 0
        while True:
            ...
            for job in jobs:
                if job.arrival_time == t:
                    ...
                    tmp_bklgs = {site.index: site.estimated_bklg_size for site in job.available_sites}
                    solution = {(site.index, tg.index): 0 for tg in job.task_groups for site in job.available_sites}

                    s = time.time()
                    # Call rd() to obtain the task assignment solution
                    _ = common.rd(job=job, tmp_bklgs=tmp_bklgs, solution=solution)
                    e = time.time()
