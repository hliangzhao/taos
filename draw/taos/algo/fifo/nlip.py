"""
The algorithm that solves the non-linear integer programming (NLIP) P.
"""
import time
import profiling
...
def NLIP(jobs, sites):
    ...
    with tqdm(total=total) as pbar:
        pbar.set_description("NLIP progress:")
        t = 0
        while True:
            ...
            for job in jobs:
                if job.arrival_time == t:
                    ...
                    s = time.time()
                    # Call obta() to obtain the task assignment solution
                    _ = common.nlip(job=job, solution=solution)
                    e = time.time()
