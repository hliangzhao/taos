...
@profiling.profiling("OBTA_2")
def OBTA_2(jobs, sites):
    total = len(jobs)
    processed = 0
    ...
    with tqdm(total=total) as pbar:
        pbar.set_description("OBTA_2 progress:")
        t = 0
        while True:
            ...
            for job in jobs:
                if job.arrival_time == t:
                    ...
                    s = time.time()
                    # Call obta_1() to obtain the task assignment solution and do it
                    _ = common.obta_2(job=job, solution=solution)
                    e = time.time()
